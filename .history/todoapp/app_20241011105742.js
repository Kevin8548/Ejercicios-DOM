...
const eliminaObjeto = e => {
    ...
    arrayTareas = arrayTareas.filter(t=> t.id !== idTarea);
    localStorage.setItem('tareas', JSON.stringify(arrayTareas));
}


const changeColor = e=> {
    const myDiv = e.target.closest('.alert');
...
arrayTareas = localStorage.getItem('tareas') ? JSON.parse(localStorage.getItem('tareas')) : [];
    creaDivs();
